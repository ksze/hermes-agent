...
def _build_user_local_paths(home: Path, path_entries: list[str]):
    return filter(
        lambda p: p not in path_entries and Path(p).exists(),
        [
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        home = Path(home_dir)
        user_local_paths = _build_user_local_paths(home, path_entries)
